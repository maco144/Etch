...
Etch is part of the **Rising Sun** portfolio managed by the AIOS internal orchestration at `***REMOVED***`. Work items (research, content, infra tasks) are dispatched here from the CEO cycle.
...
```bash
# List pending tasks
curl http://***REMOVED***:8000/api/v1/portfolio/companies/etch/work?status=pending&limit=50

# Compact view
curl -s 'http://***REMOVED***:8000/api/v1/portfolio/companies/etch/work?status=pending&limit=50' \
  | python3 -c "import sys,json; [print(i['id'][:8], i['category'], '-', i['title'][:80]) for i in json.load(sys.stdin)['items']]"

# Mark a task done
curl -X POST "http://***REMOVED***:8000/api/v1/company/work-queue/{item_id}/complete?note=what+was+done"
```
...
**Check pending tasks:**
```bash
***REMOVED*** "docker exec ***REMOVED*** psql -U <user> -c \
...
**Do NOT** rely on `http://***REMOVED***:8000` for work queue access — the kernel restarts frequently during upgrades and the API will timeout. Use PostgreSQL directly.

### Rising (Production)

- **AIOS API:** `http://***REMOVED***:8000`
